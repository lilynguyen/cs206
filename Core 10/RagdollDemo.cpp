...
void RagdollDemo::Save_Position(btRigidBody* body) {
	using namespace std;
	const btVector3 patient_zero = body->getCenterOfMassPosition();
	double position = patient_zero.getZ();
	...
	ofstream outfile;
	outfile.open("fits.dat");
	outfile << fixed << setprecision(8) << position;
	outfile.close();
		...
		if (!pause) {
		    ...
		    if (timeStep % 10 == 0) {
				for (int i = 0; i < 8; i++) {
					...
					for (int j = 0; j < 4; j++) {
						motorCommand += touches[i] * weights[j][i];
					}
